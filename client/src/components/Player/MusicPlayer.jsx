...
export default function MusicPlayer() {
  ...
  const toggleRepeat = () => {
    let mode = "off";
    if (repeatMode === "off") mode = "one";
    else if (repeatMode === "one") mode = "all";

    dispatch({
      type: actionType.SET_REPEAT_MODE,
      repeatMode: mode,
    });
  };
  ...
  const nextTrack = () => {
    ...
    if (isShuffle) {
      ...
      dispatch({ type: actionType.SET_SONG_INDEX, songIndex: randomIndex });
      return;
    }
    ...
    if (songIndex >= activeSongs.length - 1) {
      ...
      if (repeatMode === "all") {
        ...
        // Stop or Stay at end? Usually stop or pause, but here we just don't advance
        // Or loop back to 0 if we want default loop behavior
        dispatch({ type: actionType.SET_SONG_INDEX, songIndex: 0 }); // Default loop for now
      }
    } else {
  ...
  useEffect(() => {
    if ("mediaSession" in navigator && activeSongs && activeSongs[songIndex]) {
      const song = activeSongs[songIndex];
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        artist: Array.isArray(song.artist)
          ...
          : song.artist,
        album: song.album,
        artwork: [{ src: song.imageURL, sizes: "512x512", type: "image/jpeg" }],
      });
  ...
  }, [songIndex, activeSongs, isShuffle, repeatMode]); // Re-bind if logic changes due to state
        ...
        <div className="flex items-center gap-3 md:gap-4 w-full md:w-[30%] min-w-0">
          ...
          <div className="flex flex-col min-w-0 overflow-hidden">
            ...
            <p className="text-xs md:text-sm text-gray-500 truncate font-medium">
              {Array.isArray(activeSongs?.[songIndex]?.artist)
                ? activeSongs?.[songIndex]?.artist.join(", ")
          ...
          <AudioPlayer
            ...
            customAdditionalControls={[
              ...
              >
                ...
                />
                {repeatMode === "one" && (
                  <span className="absolute top-1 right-1 text-[8px] font-bold text-blue-500">
                    1
                  </span>
                )}
              </motion.button>,
            ...
            loop={repeatMode === "one"} // Native loop for single track
          />
        </div>
