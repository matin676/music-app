...
export const SongListRow = ({ data, index }) => {
  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex flex-col min-w-0">
        ...
        <span className="text-[10px] uppercase font-bold text-gray-400 border border-gray-300 px-1 rounded w-fit mt-1">
          {data.category}
        </span>
      </div>
        ...
        <div className="flex flex-col gap-2 relative">
          {filteredSongs !== null
            ? filteredSongs.map((song, i) => (
                <SongListRow key={song._id} data={song} index={i} />
              ))
            : allSongs?.map((song, i) => (
                <SongListRow key={song._id} data={song} index={i} />
              ))}
