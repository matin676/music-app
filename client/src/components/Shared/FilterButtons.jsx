...
const FilterButtons = ({ flag, filterData, multiple }) => {
  ...
  }, [flag, artistFilter, albumFilter, languageFilter, filterTerm, multiple]);
  ...
  const updateFilterButton = (value, displayName) => {
    if (multiple) {
      // Multi-select logic
      setFilterMenu(false); // Can keep open if preferred, but usually close or keep open. Let's keep open? Or close.
      // Actually for multi-select, usually you want to keep it open.
      // But the previous behavior was close. Let's keep it open for multi-select.
      setFilterMenu(true);

      if (flag === "Artist") {
        let current = Array.isArray(artistFilter) ? artistFilter : [];
        let updated = current.includes(value)
          ? current.filter((item) => item !== value)
          : [...current, value];
        dispatch({ type: actionType.SET_ARTIST_FILTER, artistFilter: updated });
      }
      if (flag === "Category") {
        let current = Array.isArray(filterTerm) ? filterTerm : [];
        let updated = current.includes(value)
          ? current.filter((item) => item !== value)
          : [...current, value];
        dispatch({ type: actionType.SET_FILTER_TERM, filterTerm: updated });
      }
    } else {
