...
const EditArtist = ({ data, close, refreshData }) => {
  const [isImageLoading, setIsImageLoading] = useState(false);
  const [alert, setAlert] = useState(null);
  const [alertMsg, setAlertMsg] = useState("");
  const [uploadProgress, setUploadProgress] = useState(0);

  const [artistName, setArtistName] = useState(data?.name || "");
  ...
  const updateArtistData = () => {
    if (!artistCoverImage || !artistName) {
      ...
      updateArtist(data._id, updatedData).then((res) => {
        if (res) {
          refreshData();
          setAlert("success");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-2xl rounded-2xl p-6 flex flex-col gap-6 shadow-2xl relative max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        <div className="flex items-center justify-between">
          <p className="text-2xl font-bold text-headingColor">Edit Artist</p>
          <MdClose
