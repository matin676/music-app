...
export default function SongCard({ data, type, index }) {
  const { refreshAllData, alertConfig } = useData();
  ...
  return (
    ...
    >
        ...
        {data.artist && (
          <span className="block text-sm text-gray-400 my-1">
            {Array.isArray(data.artist)
              ? data.artist.join(", ").length > 20
                ? `${data.artist.join(", ").slice(0, 20)}...`
                : data.artist.join(", ")
      ...
      <div className="w-full absolute bottom-2 right-2 flex items-center justify-between px-4">
        <motion.i
          whileTap={{ scale: 0.75 }}
          className="text-base text-gray-400 drop-shadow-md hover:text-headingColor"
          onClick={(e) => {
            e.stopPropagation();
