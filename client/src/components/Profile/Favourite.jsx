...
export default function Favourite() {
  ...
  // const [favorites, setFavorites] = useState([]); // Removed local state

  // UseEffect removed as we rely on global 'favourites' state
...
export const FavouriteContainer = () => {
  const [{ isSongPlaying, songIndex, allSongs, user, favourites }, dispatch] =
    useStateValue();
  ...
  const addSongToContext = (localIndex) => {
    ...
    const favoriteSongs = favourites
  ...
  return (
    <>
      <div className="col-span-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 md:gap-4 lg:gap-6 w-full">
        {favourites?.map((songId, localIndex) => {
